...
present in this repository. There is no GUI/native-runtime screenshot or
benchmark. The offline-report section provides narrower executable evidence:
`report_evidence.py` runs `report_export.py` against a fixed synthetic SQLite
fixture, verifies the resulting bundle, and a pinned headless Chromium renders
its real static index. That capture is not evidence of video processing,
recognition, camera, GUI, DTK, accuracy, performance, or production behavior.
...
The tracked demonstration executes the real standalone exporter twice against
one fixed synthetic SQLite fixture, verifies the immutable result, and binds
every public byte to a canonical receipt. The observed fixture result is **4
report-local records, 9 observation rows, 8 scored observations, 2
review-flagged records, and 82.7% aggregate confidence**. These are synthetic
exporter results, not recognition accuracy, a benchmark, or surveillance
output.
...
sha256sum docs/assets/offline-report-browser.png
# Inspect the actual PNG and update its reviewed binding constants if it changed.
python3 report_evidence.py --write
python3 report_evidence.py --check
```

`--check` never launches Docker or a browser and never changes tracked
artifacts. It regenerates the fixed export in a private repository-local
temporary directory, byte-compares the result, and cleans that temporary state
best effort. The capture script uses the already-cached image
`mcr.microsoft.com/playwright@sha256:2f29369043d81d6d69a815ceb80760f55e85f5020371ad06a4d996f18503ad1c`
with `--pull=never`, no container network, a non-root user, a read-only root,
dropped capabilities, bounded resources, and only the exact verified bundle
and a fresh output directory mounted. Chromium's own sandbox is unavailable on
this host and is explicitly disabled; the container and Docker host remain
trust boundaries. Pixel-byte equality is claimed only for the documented
browser, container/font environment, viewport, locale, and host rendering
conditions. A changed PNG hash must be visually reviewed before its capture
bindings are updated. The tooling never auto-accepts it.

[![Actual pinned-Chromium rendering of the fixed synthetic offline report](docs/assets/offline-report-browser.png)](docs/assets/offline-report-browser.png)

This is an actual 1440×2200 page-only Chromium capture of the verified
synthetic report. It shows the redacted aggregate cards, generated distribution
chart, four report-local records, and interpretation limits. It is not a
Tkinter, DTK, video, LPR, or camera screenshot.

[![Exact offline-report evidence CLI receipt](docs/assets/offline-report-cli.svg)](docs/assets/offline-report-cli.svg)

The terminal visual is rendered line-for-line from the same evidence model as
the CLI. `PASS` describes the executed fixed fixture and verified bundle;
the GUI binding is an AST check, while the browser capture is separately bound
to the exact report, HTML, generated SVG, capture script, image, and browser
binary hashes.

[![Executed synthetic report and capture flow](docs/assets/offline-report-flow.svg)](docs/assets/offline-report-flow.svg)

The workflow diagram distinguishes executed exporter/browser stages from the
unexecuted GUI and native stack. Network isolation does not turn the legacy
application into a sandboxed or supported product.

[![Redacted-v1 selected and omitted fields](docs/assets/offline-report-privacy.svg)](docs/assets/offline-report-privacy.svg)

The privacy visual is derived from the observed canonical summary and the
exporter's fixed field-selection contract. Redaction reduces exposure but does
not guarantee anonymity.

[![Content-addressed chart generated by the real exporter](docs/demo/offline-report-v1/corporatehub-report-20f6b7acf55a0a7cb53ecd650febaf21c90c4abdd299c16d9a8d06ad02b52af1/assets/af06cf68d0dd042c32d30ea05e85360837fb4bce6b329ef3c48744d74465a327.svg)](docs/demo/offline-report-v1/corporatehub-report-20f6b7acf55a0a7cb53ecd650febaf21c90c4abdd299c16d9a8d06ad02b52af1/assets/af06cf68d0dd042c32d30ea05e85360837fb4bce6b329ef3c48744d74465a327.svg)

The final chart is not a hand-authored mockup: it is the exact
content-addressed SVG inside the verified demo bundle. Review the
[canonical evidence receipt](evidence/offline-report-v1.json),
[exact synthetic report index](docs/demo/offline-report-v1/corporatehub-report-20f6b7acf55a0a7cb53ecd650febaf21c90c4abdd299c16d9a8d06ad02b52af1/index.html),
and [bound manifest](docs/demo/offline-report-v1/corporatehub-report-20f6b7acf55a0a7cb53ecd650febaf21c90c4abdd299c16d9a8d06ad02b52af1/manifest.json).
...
locations and file classes. The sole report exception is the generator-owned,
fixed synthetic evidence under `docs/demo/offline-report-v1`, `docs/assets`,
and `evidence`; it accepts no operator database. Offline report parents are
operator-selected; save every operator/runtime bundle outside the checkout and
inspect its canonical manifest before sharing. See [SECURITY.md](SECURITY.md)
...
the project-owned `path_policy`, `rtsp_policy`, `rtsp_evidence`,
`report_export`, and `report_evidence` modules. It executes the exporter only
against temporary fixed synthetic SQLite data. That set does not import the
GUI, mutable database wrapper, vendor wrappers, or native runtime:
...
policy, and tamper with bundle files and manifests. Evidence tests byte-compare
the receipt, four meaningful visuals, and exact demo bundle; validate PNG
chunks and capture provenance; mutate every artifact; and exercise fail-closed
writer and symlink cases. The standard test command does not launch Docker or
Chromium; pinned recapture is the separate reviewed command above. These checks
do not validate GUI behavior, DTK licensing, recognition quality,
native-library loading, video processing, descriptor-level filesystem race
resistance, real surveillance data, or RTSP operation.

## Repository status

This is a source-led rehabilitation stage with one narrow synthetic exporter
execution and browser-rendering path. Planned follow-up work must preserve the
vendor boundary, add reproducible dependency/runtime fixtures where licensing
permits, repair remaining behavior behind tests, and capture only real,
